...
const getNextNavigationIndex = (
  ...
  if (keyCode === 'ArrowRight') {
    return currentFocusedIndex < videogamesLength - 1
...
const getPreviewStartIndex = (
  keyCode: string,
  baseIndex: number,
  wasFocusedAtKeyPress: boolean,
  videogamesLength: number,
  focusedIndexRef: { current: number }
) => {
  if (baseIndex === -1) {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      navigationTimeoutRef.current = setTimeout(() => {
        if (pressedKeysRef.current.has(keyCode)) {
          ...
          let currentPreviewIndex = getPreviewStartIndex(
            keyCode,
            baseIndex,
            wasFocusedAtKeyPress,
            videogamesLength,
            focusedIndexRef
          )
          ...
          repeatIntervalRef.current = setInterval(() => {
            ...
            currentPreviewIndex =
              (currentPreviewIndex + direction + videogamesLength) %
              videogamesLength
            setHoveredIndex(currentPreviewIndex)
            hoverIndexRef.current = currentPreviewIndex
          }, PREVIEW_SCROLL_INTERVAL)
    ...
    const handleKeyUp = (e: KeyboardEvent) => {
      ...
      if (
        ...
        !isInPreviewModeRef.current
      ) {
        clearTimeout(navigationTimeoutRef.current)
        navigationTimeoutRef.current = null

        const nextIndexOrUnfocus = getNextNavigationIndex(
          keyCode,
          lastFocusedIndexRef.current,
          videogamesLength
        )
