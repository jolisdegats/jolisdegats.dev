...
import CloudAnimation from '@/components/Background/CloudAnimation'
...
const Background = ({ onLoadingComplete }: BackgroundProps) => {
  ...
  const loadedImagesRef = useRef<
    Record<(typeof CRITICAL_IMAGES)[number], boolean>
  >({
    background: false,
    gifCode: false,
    gifTyping: false,
  ...
  const handleImageLoad = (imageId: (typeof CRITICAL_IMAGES)[number]) => {
    ...
    const allLoaded = CRITICAL_IMAGES.every(
      (id) => loadedImagesRef.current[id] === true
    )
    if (allLoaded) {
      setIsLoading(false)
      onLoadingComplete?.()
