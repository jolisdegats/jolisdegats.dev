  ...
  ({ coffeeHeight, onClickOnMug, gameState = 'RUN' }, mugRef) => {
    ...
    return (
      <button
        ...
        className={`h-[90px] cursor-default ${gameState !== 'OFF' && coffeeHeight > 0 ? 'cursor-pointer' : ''}`}
      >
        <div
          ref={mugRef}
          className="z-[2] absolute bottom-0 left-0 bg-[#f0e3d4] w-[70px] h-[90px] rounded-[5px_5px_10%10%] transition-[opacity,left] duration-[0.3s,1s] -translate-x-1/2"
        />
        <div
          ref={coffeeRef}
          style={{
            transform: `scaleY(${Math.min(coffeeHeight / (mugRef && 'current' in mugRef ? (mugRef.current?.clientHeight ?? 1) : 1), 1)})`
          }}
