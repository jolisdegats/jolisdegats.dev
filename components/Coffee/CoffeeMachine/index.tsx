...
import Mug from '@/components/Coffee/CoffeeMachine/Mug'
...
const CoffeeMachine = forwardRef<CoffeeMachineRef, CoffeeMachineProps>(
  ({ handleStateChange, hideControls = false }, ref) => {
    ...
    const { play: playCoffeePouring, stop: stopCoffeePouringSound } =
      useSoundEffect(coffeePouring)
    ...
    const calculatePercentage = useCallback((): number => {
      return mugRef.current
        ? Math.round((coffeeHeight / mugRef.current.clientHeight) * 100)
        : 0
    }, [coffeeHeight])
    ...
    const resetGame = useCallback(
      ({
        gameState = 'PAUSED',
        shouldGetNewObjective = false
      }: ResetGameParams) => {
        const newObjective = shouldGetNewObjective
          ...
          <div className="relative w-[90%] h-[60px] mx-auto rounded-[18px_18px_10px_10px] bg-[#0e5f82] overflow-hidden py-[15px] px-0 shadow-[inset_10px_-27px_4px_#0b4a66]">
            ...
            <div className="flex justify-around items-center w-full h-full relative z-[1] text-white bottom-[10px] text-center">
              {!hideControls && (
                <div>
                  <p>
                    Fill the coffee cup {!!objective && `to ${objective}%`}{' '}
                    <br />
                    <small>
                      {[result, message].filter(Boolean).join(' - ')}
                    </small>
                  </p>
                </div>
              )}
