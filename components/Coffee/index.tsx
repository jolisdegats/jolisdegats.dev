...
import CoffeeMachine, {
  type CoffeeMachineRef,
  type HandleStateChange
} from '@/components/Coffee/CoffeeMachine'
...
export const MarkerCoffee = () => {
  ...
  const [gameState, setGameState] = useState<HandleStateChange>({
    gameState: 'OFF',
    objective: 0,
    message: '',
    result: '',
  ...
  return (
    <>
      <PopupModal
        handleClose={() =>
          coffeMachineRef.current?.resetGame({ gameState: 'OFF' })
        }
        name="coffee"
      >
        <div className="grid w-full h-full md:grid-cols-[3fr_5fr] grid-cols-1 overflow-auto">
          <div className="flex flex-col items-center text-center">
            ...
            {!gameState.objective ? (
              ...
              <p>
                Target: Fill the mug to{' '}
                <span className="font-bold">{gameState.objective}%</span>
              </p>
            )}
              ...
              <div className="mt-12 mb-4 flex flex-col items-center max-h-[50vh]">
                <div className="relative cursor-pointer w-[90px] h-[90px]">
                  <div className="w-[90px] h-[90px] left-[36px] absolute">
                    <Mug
                      onClickOnMug={() =>
                        coffeMachineRef.current?.resetGame({
                          gameState: 'PAUSED',
                          shouldGetNewObjective: true
                        })
                      }
                      coffeeHeight={gameState.coffeeHeight / 100}
                    />
                  </div>
                </div>
              </div>
